...
public class quatMove : MonoBehaviour {
	...
	public GameObject heartParticleOrigin;
	...
	//	handOrigin = GameObject.Find ("handCube").transform.position;
//		follower.transform.RotateAround (main.transform.position, Vector3.up, angle);
	...
	void FixedUpdate () {
		...
		print ("Head position: " + head.transform.position);

		heartParticleOrigin.transform.position = new Vector3 (head.transform.position.x, head.transform.position.y-.3f, head.transform.position.z);
		heartParticleOrigin.transform.eulerAngles = new Vector3 (0, 0, 0);

		/* MY OLD PLAN I DEMOED
